...
namespace library\wuti\database;
...
final class dbMysqli
{
	...
	public function get_row($one=false){
        $datalist = [];
        while (($rs = $this->fetch_next()) != FALSE) {
            $datalist[] = $rs;
        }
        ...
        if($one){
        	$datalist = isset($datalist[0]) ? $datalist[0] : $datalist;
		}
	...
	public function halt($message = '', $sql = '')
	{
		...
		if (isset($this->config['debug']) && $this->config['debug']) {
			...
			$error['msg'] = $msg;
			$this->close();
			\library\wuti\log\Logger::log($msg, 'mysql_exception');
