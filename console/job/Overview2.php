...
class Overview2 extends Job{
    public function run(){
        ...
        $channelList = functions::getChannelList($gameId);
        list($mysql,$table) = FDB::gm('report_data');

        list($mysql_zzdr,$table2) = FDB::zzdr('prop64');
        ...
        foreach ($channelList as $channelId =>$channelName){
            ...
            $ret = $mysql_zzdr->query($sql)->get_row(true);
            ...
            foreach ([1,3,7,30] as $day){
                ...
                $stayCnt = functions::getStayData($mysql_zzdr,$startTime,$channelId,$day);
