...
class Index extends Admin
{
    ...
    public function public_select_game(){
        $gameId = input('game_id',1,'intval');
        $model = model('Users');
        $model->setGameSess($gameId);
        model('Menu')->init_session_menu();
        //$url = Session::get('gm_http_referer');
        $url = '';
    ...
    public function login(){
        ...
        if(Session::has($this->session_name)){
    ...
    public function loginOut(){
        $userInfo = Session::get('gm_admin');
        Session::clear();
        model('Users')->save(['token'=>''],['id'=>$userInfo['id']]);
