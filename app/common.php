...
function set_admin_session($value){
    ...
    return \think\Session::set($session_name,$value);
}

function get_admin_session(){
    $session_name = Config::get('app.session_name');
    $session = \think\Session::get($session_name);
    if(func_num_args()){
        $args = func_get_args();
        foreach ($args as $argv){
            if(isset($session[$argv])){
...
function clear_admin_session(){
    $session_name = Config::get('app.session_name');
    return \think\Session::delete($session_name);
}

function is_login(){
    $session_name = Config::get('app.session_name');
    return \think\Session::has($session_name);
}
